...
export function AnalyticsCharts({ distribution, timelineLabels, timelineData }: AnalyticsProps) {
  ...
  // Données pour le graphe en barres (Notes 1 à 5)
  ...
  const getBarColor = (index: number) => {
    if (index < 2) return "#EF4444"; // Rouge (1-2)
    if (index === 2) return "#F59E0B"; // Orange (3)
    return "#10B981"; // Vert (4-5)
  };

  // ✅ ACTION : Clic sur une barre -> Filtre les avis
  const handleBarClick = (data: any) => {
    if (data && data.rating) {
        router.push(`/dashboard/reviews?rating=${data.rating}`);
    }
  };

  // Transformation des données timeline (Simulation pour l'exemple UI, 
  // idéalement ces données viendraient du serveur selon le filtre choisi)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      {/* 1. RÉPARTITION DES NOTES (CLIQUABLE) */}
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-gray-700 dark:text-gray-200">Répartition des Notes</h3>
            <span className="text-xs text-gray-400 flex items-center gap-1"><Filter className="w-3 h-3"/> Clic pour filtrer</span>
        </div>
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={barData} layout="vertical" margin={{ left: 0, right: 30 }}>
              ...
              />
              <Bar dataKey="count" radius={[0, 4, 4, 0]} barSize={20} onClick={handleBarClick} style={{ cursor: 'pointer' }}>
      ...
      {/* 2. ÉVOLUTION TEMPORELLE (AVANCÉE) */}
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
